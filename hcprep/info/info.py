...
class basics:
    ...
    def __init__(self):
        ...
        self.n_states_per_task = {
            task: len(self.states_per_task[task]) 
            for task in self.tasks
        }
        
        self.n_states_total = 20
        
        self.onehot_idx_per_task = dict(
        ...
        self.subjects = {
            task: np.load(
                os.path.join(
                    os.path.dirname(hcprep.__file__),
                    'info',
                    'subject_ids',
                    'task-{}_subjects.npy'.format(task)))
            for task in self.tasks
        }            
