...
def path_bids_func_mni(subject, task, run, path):
    ...
    Args:
        ...
        path: Path to local BIDS directory.
    """
    return os.path.join(path, "sub-{}".format(subject), "func", "sub-{}_task-{}_run-{}_space-MNI152NLin6Asym_desc-prepoc_bold.nii.gz".format(subject, task, run))
...
def path_bids_anat_mni(subject, task, run, path):
    ...
    Args:
        ...
        path: Path to local BIDS directory.
    """
    return os.path.join(path, "sub-{}".format(subject), "func", "sub-{}_task-{}_run-{}_space-MNI152NLin6Asym_desc-prepoc_anat.nii.gz".format(subject, task, run))
...
def path_bids_func_mask_mni(subject, task, run, path):
    ...
    Args:
        ...
        path: Path to local BIDS directory.
    """
    return os.path.join(path, "sub-{}".format(subject), "func", "sub-{}_task-{}_run-{}_space-MNI152NLin6Asym_desc-prepoc_brainmask.nii.gz".format(subject, task, run))



def path_bids_tfr(subject, task, run, path):
